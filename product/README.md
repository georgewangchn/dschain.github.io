...
## 一、简介
DSChain数据服务链是上海九豪科技网络服务有限公司开发和维护的开源联盟区块链。DSChain从源头入手对数据服务行业上下游产业链实现个人信息脱敏传输与脱敏存储、信息来源与流向追溯、资质审核平台。现行数据服务处理方式容易造成信息泄漏、非法留存、来源不明和去向流不明、商户资质不明、数据源资质不明、中间商资质不明等潜在风险。DSChain使用成熟加密算法将数据源与商户之间的信息通过一户一密的脱敏加密方式进行传输和存储，并使用区块链的方式进行资质审核与信息溯源。
...
**脱敏密钥**：是特指机构间个人信息字段的加密密钥；机构间通讯时先进行个人信息字段加密，再经通讯加密。  
...
2020年国家互联网信息办公室关于《数据安全管理办法（征求意见稿）》公开征求意见的通知。
...
DSChain在供应链数据体系内，为数据源提供个人信息脱敏加密密钥管理工具、资质审核工具，实现商户、中间商和数据源之间的加密密钥管理和资质审核。  
...
-|商户资质|商户资质入网资料透明|商户资质不明
-|数据来源|信息来源十分明确|来源不明
-|数据源资质|数据源资质入网资料透明|数据源资质不明
